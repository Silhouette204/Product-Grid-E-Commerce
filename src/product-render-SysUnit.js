import { sysUnitData } from './product-datas.js';

export function renderSysUnit(){
  const sysUnitContainer = document.querySelector("#sys-unit-container");

  if(!sysUnitContainer){
    return;
  }

  sysUnitContainer.innerHTML = sysUnitData.map(({id, productImage, brand, cpu, gpu, ram, storage, price}) => {
    
    // FIX: Dahil walang 'model' sa destructuring mo, 
    // gagamitin natin ang 'brand' o kaya 'cpu' bilang fallback model
